...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (type === "clubs" || type === "gapping") {
      const clubs = await prisma.club.findMany({ where: { isActive: true, userId } });
      const where = sessionId
        ? { sessionId, validity: "valid" as const }
        : { validity: "valid" as const, session: { userId } };
    ...
    if (type === "club" && clubId) {
      const club = await prisma.club.findFirst({ where: { id: clubId, userId } });
    ...
    const totalShots = await prisma.shot.count({
      where: { validity: "valid", session: { userId } },
    });
